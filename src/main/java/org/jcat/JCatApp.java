...
public class JCatApp {
	...
	public void exec() throws IOException {
		...
		if ( pluginHolder.existsHelpPlugins() ) {
			pluginHolder.showOptions();
		} else {
			for (String path : option.getFileList()) {
				FileInput input = new FileInput(path);
				try (IJCatStream<FileInput, StandardOutput> stream = new JCatStream<FileInput, StandardOutput>(this.option,
						input, this.output)) {
					while (stream.next()) {
